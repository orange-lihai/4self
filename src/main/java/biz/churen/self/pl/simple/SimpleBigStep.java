 ...
 */
public class SimpleBigStep {
    public Struct expression;
    public Map<String, Struct> env;
    public SimpleBigStep(Struct exp) {
        this.expression = exp;
    }

    public SimpleBigStep(Struct exp, Map<String, Struct> env) {
        this.expression = exp;
        this.env = env;
    }
    ...
    public static void main(String[] args) {
        ...
        SimpleBigStep simple = new SimpleBigStep(exp);
        simple.run();
        ...
        simple = new SimpleBigStep(exp);
        simple.run();

        Map<String, Struct> env = new HashMap<>();
        env.put("b", new Bool(false));
        env.put("x", new Num(5));
        env.put("y", new Num(6));
        ...
        simple = new SimpleBigStep(_if, env);
        simple.run();


        env.clear();
        env.put("x", new Num(1));
        simple = new SimpleBigStep(
            new While(
